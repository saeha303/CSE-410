...
public:
    ...
    void draw()
    {
        ...
        glutSolidSphere(0.5, 10, 10); // Draw a sphere with radius 1.0
        glPopMatrix();
    }
...
public:
    ...
    Object() {}
    virtual void draw() = 0;
    virtual double getIntersectionPoint_t(Ray ray, double *color, int level) = 0;
    ...
    virtual double intersect(Ray ray, double *color, int level)
    {
        ...
        vector<double> intersectionPointColor = getColorAt(intersectionPoint);
        ...
        for (PointLight *pl : pointLights)
        {
            ...
            if (!isInShadow)
            {
                ...
                double phong = max(0.0, -(ray.dir * ray_r.dir));
                color[0] += pl->color[0] * coefficients[1] * lambert * intersectionPointColor[0] + pl->color[0] * coefficients[2] * pow(phong, shine) * intersectionPointColor[0];
                color[1] += pl->color[1] * coefficients[1] * lambert * intersectionPointColor[1] + pl->color[1] * coefficients[2] * pow(phong, shine) * intersectionPointColor[1];
                color[2] += pl->color[2] * coefficients[1] * lambert * intersectionPointColor[2] + pl->color[2] * coefficients[2] * pow(phong, shine) * intersectionPointColor[2];
            }
        }
        for (SpotLight *sl : spotLights)
        {
            ...
            if (fabs(angle) < sl->cutoff_angle)
            {
                ...
                if (!isInShadow)
                {
                    ...
                    // find reflected ray
                    double phong = max(0.0, -(ray.dir * ray_r.dir));
                    color[0] += sl->point_light.color[0] * coefficients[1] * lambert * intersectionPointColor[0] + sl->point_light.color[0] * coefficients[2] * pow(phong, shine) * intersectionPointColor[0];
                    color[1] += sl->point_light.color[1] * coefficients[1] * lambert * intersectionPointColor[1] + sl->point_light.color[1] * coefficients[2] * pow(phong, shine) * intersectionPointColor[1];
                    color[2] += sl->point_light.color[2] * coefficients[1] * lambert * intersectionPointColor[2] + sl->point_light.color[2] * coefficients[2] * pow(phong, shine) * intersectionPointColor[2];
                }
            }
        }
        if (level < recursionLevel)
        {
            ...
            if (obj != NULL)
            {
                double *colorReflected = new double[3]; // refelction color
                for (int i = 0; i < 3; i++)
                    colorReflected[i] = 0;
